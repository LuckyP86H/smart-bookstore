...
func (d *Database) initSchema() error {
	// First, enable pgvector extension for AI/vector search
	_, err := d.db.Exec("CREATE EXTENSION IF NOT EXISTS vector")
	if err != nil {
		// Log but don't fail if extension already exists or user doesn't have permission
		// In production, ensure pgvector is pre-installed
	...
	// Add embedding column for AI/vector search (if it doesn't exist)
	// This column stores vector embeddings generated by the AI service
	_, err = d.db.Exec(`
		DO $$ BEGIN
			ALTER TABLE books ADD COLUMN IF NOT EXISTS embedding vector(384);
		EXCEPTION
			WHEN undefined_object THEN
				-- pgvector extension not installed, skip
	...
	`)
	if err != nil {
		fmt.Printf("Warning: Could not create embedding index: %v\n", err)
	}
